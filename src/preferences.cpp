...
Preferences::Preferences ( MainWindow* parent )
{
    ...
    connect ( m_text_wrap_chkbox, SIGNAL ( toggled ( bool ) ), this, SLOT ( text_wrap_toggled ( bool ) ) );
...
//*****************************************************************************
void Preferences::get_config ( QList< QString >* config_keys, QList< QVariant >* config_values )
{
    * config_keys = * m_config_keys;
...
void Preferences::config_save ( )
{
    ...
    emit config_changed ( m_config_values );
}
...
//*****************************************************************************
void Preferences::text_wrap_toggled ( bool val )
{
    m_text_wrap_words_chkbox->setEnabled ( val );
}
...
void Preferences::config_read ( )
{
    ...
    for ( int i = 0; i < m_config_keys->size ( ); i++ )
    {
        m_config_values->append ( m_settings->value ( m_config_keys->at ( i ), m_config_default_values->at ( i ) ) );
    }
...
void Preferences::update_widgets ( )
{
    m_text_wrap_chkbox->setChecked (
            m_config_values->at ( m_config_keys->indexOf ( "text_wrap" ) ).toBool ( ) );

    if ( m_text_wrap_chkbox->isChecked ( ) )
    {
        m_text_wrap_words_chkbox->setChecked (
                m_config_values->at ( m_config_keys->indexOf ( "text_wrap_whole_words" ) ).toBool ( ) );
    }
    else
    {
        m_text_wrap_words_chkbox->setChecked ( false );
    }

    m_ln_nums_chkbox->setChecked (
            m_config_values->at ( m_config_keys->indexOf ( "show_line_numbers" ) ).toBool ( ) );

    m_curr_ln_chkbox->setChecked (
            m_config_values->at ( m_config_keys->indexOf ( "highlight_current_line" ) ).toBool ( ) );

    m_tab_width_spinbox->setValue (
            m_config_values->at ( m_config_keys->indexOf ( "tab_width" ) ).toInt ( ) );

    QStringList sl = m_config_values->at ( m_config_keys->indexOf ( "font" ) ).toString ( ).split ( ",", QString::SkipEmptyParts );
...
void Preferences::get_widgets_values ( )
{
    ...
    {
        m_config_values->replace ( m_config_keys->indexOf ( "text_wrap_whole_words" ),  0 );
    }
